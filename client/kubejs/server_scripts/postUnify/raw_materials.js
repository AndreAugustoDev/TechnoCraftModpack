...
ServerEvents.recipes((event) => {
  ...
  raw_materialTags.forEach((raw_materialTagString) => {
    ...
    let raw_material = AlmostUnified.getPreferredItemForTag(
      raw_materialTagString
    );
      ...
      if (global.loaded.IE_Loaded) {
        ...
        if (count == 0) {
          event
            ...
            .id(
              `kubejs:immersiveengineering/metalpress/raw_material_${material}`
            );
      ...
      if (global.loaded.Thermal_Loaded) {
        ...
        if (count == 0) {
          event
            ...
            .id(
              `kubejs:thermal/machines/press/unpacking/press_raw_${material}_unpacking`
            );
  ...
  if (global.devLogging) {
    console.log(
      `Added Raw Material Recipes - IE: ${raw_materialCount.ie}, Thermal: ${raw_materialCount.thermal}`
    );
