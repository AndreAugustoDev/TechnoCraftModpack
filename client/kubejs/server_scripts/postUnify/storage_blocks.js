...
ServerEvents.tags("item", (event) => {
  event.add(
    ...
    "xycraft_world:aluminum_raw_block"
  );
...
ServerEvents.recipes((event) => {
      ...
      if (global.loaded.IE_Loaded) {
        ...
        event.forEachRecipe(
          ...
          (recipe) => {
            ...
            if (result.has("base_ingredient")) {
              if (
                global.ingredientCheck(storage, result.get("base_ingredient"))
              ) {
                count++;
              }
        ...
        if (count == 0) {
          event
            ...
            .id(
              `kubejs:immersiveengineering/metalpress/packing3x3/${
                raw ? "raw_" : ""
              }${material}`
            );
          storageCount.ie++;
      ...
      if (global.loaded.Thermal_Loaded) {
        ...
        if (count == 0) {
          event
            ...
            .id(
              `kubejs:thermal/machines/press/packing3x3/press_${
                raw ? "raw_" : ""
              }${material}_packing`
            );
  ...
  if (global.devLogging) {
    console.log(
      `Added Storage Block Recipes - IE: ${storageCount.ie}, Thermal: ${storageCount.thermal}`
    );
