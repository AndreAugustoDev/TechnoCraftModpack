...
ServerEvents.recipes((event) => {
  ...
  */
  ...
  // sleeping bags
  event.forEachRecipe(
    { type: "minecraft:crafting_shaped", id: /comforts:sleeping_bag_.+/ },
    (recipe) => {
      let out = recipe.json.get("result");
      let input = recipe.json.get("key").get("a");
      event
        .shaped(Item.of(out), ["   ", "   ", "aaa"], {
          a: Ingredient.of(input),
        })
        .noMirror()
        .noShrink()
        .id(`kubejs:${recipe.getId().replace(":", "/")}`);
      event.remove({ id: recipe.getId() });
    }
  );
  // wool slabs
  event.forEachRecipe(
    { type: "minecraft:crafting_shaped", id: /absentbydesign:slab_wool_.+/ },
    (recipe) => {
      let out = recipe.json.get("result");
  ...
  event
    .shaped("structurize:sceptergold", ["C  ", " S ", "  S"], {
  ...
  event
    .shaped("constructionwand:stone_wand", ["  C", " S ", "S  "], {
      C: "#minecraft:stone_crafting_materials",
      S: "#forge:rods/wooden",
    })
    .noMirror()
    .id(`kubejs:constructionwand/stone_wand`);
  event.remove({ id: "structurize:sceptersteel" });
  event
    .shaped("structurize:sceptersteel", ["C  ", " S ", "  S"], {
      C: "#forge:ingots/iron",
      S: "#forge:rods/wooden",
    })
    .noMirror()
  ...
  event
    .shaped("constructionwand:iron_wand", ["  C", " S ", "S  "], {
      C: "#forge:ingots/iron",
      S: "#forge:rods/wooden",
    })
    .noMirror()
  ...
  // Blue Skies tools
  let planks = Ingredient.of("#minecraft:planks").subtract(
    Ingredient.of("#blue_skies:planks")
  );
  event.forEachRecipe(
  ...
  let cobble = Ingredient.of("#quark:stone_tool_materials").subtract(
    Ingredient.of("#blue_skies:cobblestone")
  );
  event.forEachRecipe(
    {
  ...
  event
    .shaped("minecraft:chest", ["PPP", "P P", "PPP"], { P: chestPlanks })
    .id("kubejs:chest");
  ...
  let convertableChests = Ingredient.of("#forge:chests/wooden")
    .subtract(Ingredient.of("#forge:chests/trapped"))
    .subtract("minecraft:chest");
  event
    .shapeless("minecraft:chest", convertableChests)
    .id("kubejs:chest/plain");

  // Rechiseled Chisel / Croptopia Knife
  event.remove({ id: "rechiseled:chisel" });
  event
    .shaped("rechiseled:chisel", ["C ", " S"], {
      C: "#forge:ingots/iron",
      S: "#forge:rods/wooden",
    })
    .noMirror()
    .id(`kubejs:rechiseled/chisel`);
  event.remove({ id: "croptopia:knife" });
  event
    .shaped("croptopia:knife", [" C", "S "], {
      C: "#forge:ingots/iron",
      S: "#forge:rods/wooden",
    })
    .noMirror()
    .id(`kubejs:croptopia/knife`);

  // Mangrove ABD plank wall (match other plank wall recipes)
  event.remove({ id: "absentbydesign:wall_mangrove_planks" });
  event.shaped("absentbydesign:wall_mangrove_planks", ["   ", "###", "# #"], {
    "#": "minecraft:mangrove_planks",
  });
  ...
  event.shaped("additional_lights:fire_for_standing_torch_s", ["S", "C"], {
    S: "#forge:rods/wooden",
    C: "#minecraft:coals",
  });
  ...
  let logSticks = Ingredient.of("#minecraft:logs").subtract(
    Ingredient.of(["#botania:livingwood_logs", "#botania:dreamwood_logs"])
  );
  event
    .shaped("16x minecraft:stick", ["s", "s"], { s: logSticks })
    .id("kubejs:easy_sticks");
});
