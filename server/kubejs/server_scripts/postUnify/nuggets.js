...
ServerEvents.recipes((event) => {
      ...
      if (global.loaded.IE_Loaded) {
        ...
        event.forEachRecipe(
          ...
          (recipe) => {
            ...
            if (result.has("base_ingredient")) {
              if (
                global.ingredientCheck(nugget, result.get("base_ingredient"))
              ) {
                count++;
              }
        ...
        if (count == 0) {
          event
            ...
            .id(
              `kubejs:immersiveengineering/metalpress/unpacking/nugget_${material}`
            );
          nuggetCount.ie++;
        }
      }
      if (global.loaded.Thermal_Loaded) {
        ...
        if (count == 0) {
          event
            ...
            .id(
              `kubejs:thermal/machines/press/unpacking/press_${material}_nugget_unpacking`
            );
          nuggetCount.thermal++;
        }
      }
  ...
  if (global.devLogging) {
    console.log(
      `Added Nugget Recipes - IE: ${nuggetCount.ie}, Thermal: ${nuggetCount.thermal}`
    );
